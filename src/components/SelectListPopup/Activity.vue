<template>
  <a-modal :title="title" style="top: 100px;" :visible.sync="isShow" width="1100px" @ok="handleSubmit"
    @cancel="handleCancel">
    <div>
      ...
      <a-table class="table" :row-selection="selection" borded :rowKey="row => row.id" :dataSource="list"
        ...
        <a-table-column title="类型" width="70px">
          <template slot-scope="row">
            <div>{{row.type_text}}</div>
          </template>
        </a-table-column>
