<template>
  <a-modal :zIndex="2000" :title="title" style="top: 100px;" :visible.sync="isShow" width="1100px" @ok="handleSubmit"
    @cancel="handleCancel">
    <div>
      ...
      <a-table class="table" :row-selection="selection" borded :rowKey="row => row.id" :dataSource="list"
        ...
        <a-table-column title="编号" width="110px">
          <template slot-scope="row">
            <div>{{row.number}}</div>
          </template>
        </a-table-column>
        <a-table-column title="标题" width="300px">
          <template slot-scope="row">
            <div>{{row.title}}</div>
          </template>
        </a-table-column>
...
<script>
  const resourceType = 'shop/products'
