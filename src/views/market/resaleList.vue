<template>
  <div class="list">
    ...
    <a-table class="table" :rowKey="row => row.uuid" :dataSource="list" :pagination="pagination" :size="size">
      ...
      <a-table-column title="SID" width="10px">
      ...
      <a-table-column title="商品" width="100px">
        <template slot-scope="row">
          <div style="display: flex; align-items: center;"  v-for="(item, index) in row.package_skus.slice(0, 1)">
          ...
          <div v-if="row.package_skus.length > 2">
            查看共
            <a href="javascript:;" @click="handleShowSkuList(row)">
            {{row.product_total}}件商品
            </a>
          </div>
        </template>
      </a-table-column>
      ...
      <a-table-column title="转售价" width="40px">
        <template slot-scope="row">
          <PriceDisplay :info="row"></PriceDisplay>
        </template>
      </a-table-column>

      <a-table-column title="购买用户" width="10px">
        <template slot-scope="row">
          <div class="user" v-if="row.buyer">
            <img :src="row.buyer.headimg" alt="用户头像">
            <div>
              <div>{{ row.buyer.name }}</div>
              <div>{{row.buyer.phone}}</div>
            </div>
            <!-- <a-icon class="gender-man" v-if="row.gender==='男'" type="man" />
            <a-icon class="gender-woman" v-else type="woman" /> -->
          </div>
      ...
      <a-table-column title="自动匹配" width="60px">
        <template slot-scope="row">
          {{row.is_allow_fast_match ? '允许' : '不允许'}}
        </template>
      </a-table-column>
      ...
      <a-table-column title="成交方式" width="60px">
        <template slot-scope="row">
          <span v-if="row.complete_type === 1">用户自由交易</span>
          <span v-else-if="row.complete_type === 2">管理后台购买</span>
          <span v-else-if="row.complete_type === 3">系统自动匹配</span>
          <span v-else>-</span>
        </template>
      </a-table-column>
        ...
        <template slot-scope="row">
          <span>
            ...
            <template v-if="row.union_status === 'pending'">
              <a-divider type="vertical" />
              <a href="javascript:;" @click="handleBuy(row)">购买</a>
            </template>
          </span>
        </template>
      </a-table-column>
    ...
    <SkuDetailList @hide="isSkuDetail=false" :isShow="isSkuDetail" :info="current"></SkuDetailList>
...
<script>
  ...
  import SkuDetailList from "./components/SkuDetailList"


  const resourceType = 'asset/resales'
