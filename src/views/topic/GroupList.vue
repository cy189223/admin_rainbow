<template>
  <div class="list">
    ...
    <a-table class="table" :rowKey="row => row.uuid" :dataSource="list" :pagination="pagination" :size="size">
      ...
      <a-table-column title="帖子数量" width="40px">
        <template slot-scope="row">
          {{row.visit_total}}
        </template>
      </a-table-column>
    ...
    <a-modal :title="editUuid ? '编辑' : '创建' + resourceTypeText" style="top: 100px;" :visible="isEdit" width="1000px" @cancel="isEdit = false" :footer="null">
...
<script>
  ...
  import EditForm from "./EditGroup.vue"
  const resourceType = 'topic-groups'
  const resourceTypeText = '圈子'
