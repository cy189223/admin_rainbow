<template>
  <div class="list">
    ...
    <a-table class="table" :rowKey="row => row.uuid" :dataSource="list" :pagination="pagination" :size="size">
      <a-table-column title="内容" width="300px">
        <template slot-scope="row">
          <div>
            <span class="top-btn" v-if="row.set_top_at" @click="unsetTop(row)">已置顶</span>
            <span class="top-btn unset" v-else @click="setTop(row)">未置顶</span>
            {{row.text}}
          </div>
      ...
      <a-table-column title="话题" width="100px">
        <template slot-scope="row">
          <template v-if="row.topic_tag">
            #{{row.topic_tag.title}}#
          </template>
        </template>
      </a-table-column>
      <a-table-column title="圈子" width="100px">
        <template slot-scope="row">
          {{row.topic_group && row.topic_group.title}}
        </template>
      </a-table-column>
      <a-table-column title="阅读量" width="40px">
        <template slot-scope="row">
          {{row.visit_total}}
        </template>
      </a-table-column>
      ...
      <a-table-column title="发表日期" dataIndex="created_at" width="80px">
