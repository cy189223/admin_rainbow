<template>
  <div class="list">
    ...
    <a-table class="table" :rowKey="row => row.uuid" :dataSource="list" :pagination="pagination" :size="size">
	  ...
	  <a-table-column title="达额金额" width="40px">
	    <template slot-scope="row">
	      {{row.min_price}}
	    </template>
	  </a-table-column>
	  <a-table-column title="商品" width="180px">
        <template slot-scope="row">
          <img class="thumb" :src="row.goods_image" />
          {{row.goods_name}}
        </template>
      </a-table-column>
	  <a-table-column title="商品简介" width="100px">
	    <template slot-scope="row">
	      {{row.goods_desc}}
	    </template>
	  </a-table-column>
      <!-- 
      <a-table-column title="销量" width="40px">
        <template slot-scope="row">
          {{row.sales}}
        </template>
      </a-table-column>
...
<script>
  ...
  import EditForm from "./EditLevel.vue"
  const resourceType = 'user-levels'
  const resourceTypeText = '用户等级'
