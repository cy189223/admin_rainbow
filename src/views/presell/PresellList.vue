<template>
  <div class="list">
    ...
    <a-table class="table" :rowKey="row => row.uuid" :dataSource="list" :pagination="pagination" :size="size">
      <a-table-column title="日期" width="40px">
      ...
      <a-table-column title="图片" width="60px">
        <template slot-scope="row">
          <img @click="$viewImage(url)" :src="row.thumb" class="content-image"></img>
        </template>
      </a-table-column>
...
<script>
  ...
  import EditForm from "./EditPresell.vue"
  const resourceType = 'presell-items'
  const resourceTypeText = '预售信息'
