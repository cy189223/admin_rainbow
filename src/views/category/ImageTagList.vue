<template>
  <div class="list">
    ...
    <a-table class="table" :rowKey="row => row.uuid" :dataSource="list" :pagination="pagination" :size="size">
      <a-table-column title="角标图片" width="40px">
        <template slot-scope="row">
          <img class="thumb" :src="row.image" />
        </template>
      </a-table-column>
      <a-table-column title="角标名" width="100px">
        <template slot-scope="row">
          {{row.title}}
        </template>
      </a-table-column>
...
<script>
  ...
  import EditForm from "./EditImageTag.vue"
  const resourceType = 'image-tags'
  const resourceTypeText = '角标'
