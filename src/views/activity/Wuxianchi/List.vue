<template>
  <div class="list">
    ...
    <a-table class="table" :rowKey="row => row.uuid" :dataSource="list" :pagination="pagination" :size="size">
      ...
      <a-table-column title="编辑概率" width="100px">
        <template slot-scope="row">
...
<script>
  ...
  const resourceType = 'fudai_yi-fan-shangs'
  const resourceTypeText = '无限池'
  ...
  export default {
    ...
    methods: {
      fetchTotalData() {
        this.$http('/status-total/activity/fudai_yifanshang').then((res) => {
