...
<!--        <a-input-search placeholder="input search text" enter-button @search="onSearch" />
        ...
        <router-link to="/activity/fudai/setting" @click="handleDelete(row)" style="margin: 0px 10px;">福袋配置</router-link>
          ...
          <template v-for="n in 5">
            <div v-if="row.sku_level[n-1].status" style="margin-bottom: 4px;">
              ...
              <a href="javascript:;" @click="handleEditSkuList(row, n)">
                <span class="">{{row.sku_level[n-1].sku_total}}种奖品</span>
              </a>
...
<script>
  ...
  const resourceType = 'fudais'
  const resourceTypeText = '福袋'
  ...
  export default {
    ...
    methods: {
      fetchTotalData() {
        this.$http('/status-total/activity/fudai').then((res) => {
      ...
      handleTestLottery(row) {
        this.$iForm.show({
          ...
          success: (data) => {
            this.$http.post(`/fudais/${row.uuid}/test-lottery`, {
              ...data
            }).then(res =>  {
