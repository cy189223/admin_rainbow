...
<script>
  ...
  export default {
    ...
    methods: {
      initList() {
        this.$http.get(`/egg-lottery/skus`, {
          params: {
      ...
      handleCreate() {
        this.$iForm.show({
          ...
          frame: this.frame,
          success: (data) => {
            this.$http.post(`/egg-lottery/skus`, {
              activity_id: this.info.id,
              ...data
            }).then(res => {
              this.initList()
      ...
      handleEdit(row) {
        this.$iForm.show({
          ...
          form: row,
          success: (data) => {
            this.$http.put(`/egg-lottery/skus/${row.uuid}`, {
              type: 'update',
              attributes: data
            }).then(res => {
              this.initList()
      ...
      handleDelete(row) {
        this.$confirm({
          ...
          content: `确认删除吗？`,
          onOk: () => {
            this.$http.delete(`/egg-lottery/skus/${row.uuid}`).then(res => {
