<template>
  <div class="container">

    <a-tabs default-active-key="1">
      <a-tab-pane key="1" tab="基础设置">
        <IForm :formFrame="frameBase" class="form-container" :formData.sync="form"></IForm>
      </a-tab-pane>
      <!-- <a-tab-pane key="2" tab="页面装修" force-render>
        <IForm :formFrame="frameUI" class="form-container" :formData.sync="form"></IForm>
      </a-tab-pane> -->
    </a-tabs>

    <!-- <IForm :formFrame="frame" class="form-container" :formData.sync="form"></IForm> -->
    <div slot="action" style="display: flex; justify-content: flex-end;">
      <a-button class="btn" @click="cancel">取消</a-button>
      <a-button class="btn" type="primary" @click="submit">提交</a-button>
    </div>
    <!-- <AttrEditor :form.sync="form">
			<div slot="action">
				<a-button class="btn" @click="cancel">取消</a-button>
				<a-button class="btn" type="primary" @click="submit">提交</a-button>
			</div>
		</AttrEditor> -->

  </div>
</template>

<script>
  // import AttrEditor from "./components/AttrEditor"
  import IForm from "@/components/IForm"

  const resourceType = 'pks'

  export default {
    components: {
      IForm
      // AttrEditor
    },
    data() {
      return {
        form: {},
        // type: 'lottery_egg',
        categoryList: []
      }
    },
    props: {
      uuid: {
        type: String
      },
    },
    computed: {
      categoryOptions() {
        return this.categoryList.map(item => {
          return {
            title: item.title,
            value: item.id
          }
        })
      },
      frameBase() {
        return [{
            title: '活动标题',
            key: 'title',
            newline: true,
            required: true
          },
          // {
          //   title: '副标题',
          //   key: 'sub_title',
          //   placeholder: '例如: 直播中随机抽取10位幸运儿',
          //   newline: true,
          //   // required: true
          // },
          // {
          //   title: '参与码',
          //   key: 'join_codes',
          //   tips: '选填。参与码可设置多个，可在公众号或微信群中发放给用户',
          //   type: 'tags',
          //   newline: true,
          // },
          {
            title: '封面',
            key: 'thumb',
            type: 'image',
            tips: '800*800',
            isShowImageTips: false,
            newline: true,
            sizeLimit: 200,
            // required: true
          },
					{
						title: '分类',
						key: 'type',
						type: 'radio',
						default: 'yfs',
						required: true,
						options: [{
								title: '全局赏',
								value: 1
							},
							{
								title: 'PK',
								value: 2
							}
						],
						tips: ' <span style="color:red;">创建后不可更改</span>',
						disabled: this.uuid,
						newline: true,
					},
					{
						title: '串串奖励',
						type: 'number',
						input_width: '40px',
						key: 'join_num',
						placeholder: '',
						default:0,
						prefix: '玩家一局内购买单发获得',
						suffix: '个以上全局奖品额外赠送奖品',
						tips: ' 0为不使用串串奖励，<span style="color:red;">创建后不可更改</span>',
						disabled: this.uuid,
						where: {
							type: 1
						}
					},
					{
					  title: '参与人数',
					  key: 'person_num',
					  type: 'number',
					  newline: true,
					  disabled: this.uuid,
					  // tips: '',
					  where: {
					  	type: 1
					  },
					  required: true,
					  tips: ' <span style="color:red;">创建后不可更改</span>',
					},
					{
					  title: 'A阵营人数',
					  key: 'person_num',
					  type: 'number',
					  newline: true,
					  disabled: this.uuid,
					  // tips: '',
					  where: {
					  	type: 2
					  },
					  required: true,
					},
			{
			  title: '售价',
			  key_prefix: '',
			  type: 'mix_price',
			  newline: true,
			  where: {
			  	type: 1
			  },
			},
			{
			  title: 'A阵营售价',
			  key_prefix: '',
			  type: 'mix_price',
			  newline: true,
			  where: {
			  	type: 2
			  },
			},
			{
			  title: 'B阵营售价',
			  key_prefix: 'other_',
			  type: 'mix_price',
			  newline: true,
			  where: {
			  	type: 2
			  },
			},
          {
            title: '标签',
            key: 'tags',
            type: 'tags',
            tips:  '在列表标题下方显示，如“热卖新品”等短语。 填写多个仅显示第一个',
            newline: true,
            // required: true,
          },
          // {
          //   title: '轮播图',
          //   key: 'images',
          //   type: 'images',
          //   tips: '560*210',
          //   isShowImageTips: false,
          //   newline: true,
          //   required: true
          // },
          // {
          //   title: '3D立体图/盒图',
          //   key: 'image_3d',
          //   type: 'image',
          //   isShowImageTips: false,
          //   tips: '500 * 500 png  不上传则使用默认',
          //   sizeLimit: 300,
          //   newline: true,
          // },
          // {
          //   type: 'blank',
          //   newline: true,
          // },
          // {
          //   title: '开放参与时间',
          //   key: 'start_at',
          //   type: 'date',
          //   newline: true,
          //   required: true,
          // },
          
          // {
          //   title: '进回血池最低欧气值',
          //   key: 'min_lucky_score',
          //   type: 'number',
          //   newline: true,
          //   tips: '当用户的欧气值大于此值时将进入回血池抽奖品 <br/> 不填或填0为不设置，回血池中的奖品将永远不会送出',
          //   // required: true,
          // },
          // {
          //   title: '连开优惠',
          //   key: 'multi_buy_discount',
          //   type: 'multi_open_discount',
          //   newline: true,
          //   // tips: '不设置则没有连'
          //   // required: true
          // }
        ]
      },
      frameUI() {
        return [
          {
            title: '机柜主题风格',
            key: 'theme',
            type: 'radio',
            default: 1,
            tips: '当前仅默认主题可用',
            options: [
              {
                title: '默认主题',
                value: 1
              }
            ],
            newline: true,
            required: true
          },
          {
            title: '页面背景图',
            key: 'bg_image',
            tips: '不上传则使用默认图',
            isShowImageTips: false,
            tips: '1000 * 2160，不上传则使用默认图',
            type: 'image',
            newline: true,
          },
          {
            title: '机柜图',
            key: 'body_image',
            tips: '750*1200, 参考图片规范包中扭蛋机机身图, 不上传则使用默认',
            isShowImageTips: false,
            type: 'image',
            newline: true,
          },
          {
            title: '扭1次按钮',
            key: 'btn_1',
            tips: '200*80, 参考图片规范包中对应图片, 不上传则使用默认',
            isShowImageTips: false,
            type: 'image',
            newline: true,
          },
          {
            title: '扭1次按钮（按下）',
            key: 'btn_1_on',
            tips: '200*80, 参考图片规范包中对应图片, 不上传则使用默认',
            isShowImageTips: false,
            type: 'image',
            newline: true,
          },
          {
            title: '扭5次按钮',
            key: 'btn_5',
            tips: '126*66, 参考图片规范包中对应图片, 不上传则使用默认',
            isShowImageTips: false,
            type: 'image',
            newline: true,
          },
          {
            title: '扭5次按钮（按下）',
            key: 'btn_5_on',
            tips: '126*66, 参考图片规范包中对应图片, 不上传则使用默认',
            isShowImageTips: false,
            type: 'image',
            newline: true,
          },
          {
            title: '试一试按钮',
            key: 'btn_try',
            tips: '126*66, 参考图片规范包中对应图片, 不上传则使用默认',
            isShowImageTips: false,
            type: 'image',
            newline: true,
          },
          {
            title: '试一试按钮（按下）',
            key: 'btn_try_on',
            tips: '126*66, 参考图片规范包中对应图片, 不上传则使用默认',
            isShowImageTips: false,
            type: 'image',
            newline: true,
          },
        ]
      }
    },
    created() {},
    mounted() {
      this.uuid = this.uuid || this.$route.params.uuid

      // this.$http.get('/article/categories').then(res => {
      // 	this.categoryList = res.data.list
      // })

      this.initForm()
    },
    watch: {
      uuid() {
        this.initForm()
      }
    },
    methods: {
      initForm() {
        if (this.uuid) {
          this.$http(`/${resourceType}/${this.uuid}`).then(res => {
            this.form = res.data.info
          })
        } else {
          this.form = {
            ...this.$initForm(this.frameBase),
            // ...this.$initForm(this.frameUI)
          }
        }
      },
      cancel() {
        this.uuid = ''
        this.$emit('cancel')
        // this.$router.back(-1)
      },
      create() {
        this.$http.post(`/${resourceType}`, {
          ...this.form,
          status:0,
          list_weight:999
        }).then(res => {
          this.$message.success('创建成功')
          this.$emit('success')
          // this.$router.push({
          // 	path: './list'
          // })
        })
      },
      update() {
        this.$http.patch(`/${resourceType}/${this.uuid}`, {
          type: 'update',
          attributes: this.form
        }).then(res => {
          this.$message.success('更新成功')
          this.$emit('success')
          // this.uuid = ''
          // this.form = {}

          // this.$router.push({
          // 	path: '../list'
          // })
        })
      },
      submit() {
        if (this.uuid)
          this.update()
        else
          this.create()
      }
    }
  }
</script>

<style lang="scss" scoped>
  .form-container {
    padding-bottom: 120px;
  }

  .container {
    .btn {
      margin-right: 30px;
    }
  }
</style>
