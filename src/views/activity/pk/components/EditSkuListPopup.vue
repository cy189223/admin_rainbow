	...
	<a-modal :title="title" style="top: 100px;" :visible="true" width="1100px" @cancel="handleCancel">
		...
		<a-table class="product-table" :rowKey="row => row.id" :dataSource="list" bordered :pagination="false"
			...
			<a-table-column title="类型" key="number" width="100px">
				<template slot-scope="row">
					<div>{{ row.prize_type == 'A' ? 'A阵营保底' : (row.prize_type == 'B' ? 'B阵营保底' : '全局奖品') }}</div>
				</template>
			</a-table-column>
		...
		<template slot="footer">
			<div class="btn-c">
				<a-button type="primary" class="add" @click="handleCreate" v-if="list.length  == 0 || this.currentSkuLevel == 'N'">
					新增一款
				</a-button>
...
<script>
	...
	export default {
		...
		computed: {
			title() {
				console.log(this.info,this.currentSkuLevel)
				if(this.info.type == 1){
					return this.currentSkuLevel == 'N' ? '全局奖品' : (this.currentSkuLevel == 'A' ? '保底奖品' : '串串奖励')
				}
				return this.currentSkuLevel == 'N' ? '全局奖品' : (this.currentSkuLevel == 'A' ? 'A阵营保底奖品' :'B阵营保底奖品')
		...
		methods: {
			initList() {
				this.$http.get(`/pks/${this.info.uuid}/skus`, {
					params: {
						prize_type: this.currentSkuLevel,
			...
			handleCreate() {
				this.$iForm.show({
					...
					frame: this.frame,
					success: (data) => {
						this.$http.post(`/pks/${this.info.uuid}/skus`, {
							...data,
							activity_id:this.info.id,
