<template>
  <a-modal :title="list.type + (list.type == 'A' ? list.seat_code : '阵营') + (list.user && list.user.name || '')" style="top: 100px;" :visible="true" width="1100px" @cancel="handleCancel">
	<!-- {{list.type}} {{list.user}}{{(list.type == 'A' ? list.seat_code : '阵营')}} -->
	<!-- list.user && list.user.name || list.type + '阵营' -->
    <!-- <div>
    ...
    <a-table class="product-table table" :pagination="pagination" :rowKey="row => row.id" :dataSource="list.skus" bordered
      size="small">
      ...
      <a-table-column title="奖品" key="thumb" width="80px">
		  <template slot-scope="row">
		  	<div style="display: flex; align-items: center;">
		  		<img :src="row.sku.thumb" class="thumb" style="margin-right: 10px;" />
		  		<div>
		  			{{row.sku.title}} <br />
		  			{{row.sku.number}}
		  		</div>
      ...
      <a-table-column title="详情" width="150px">
        <template slot-scope="row">
			<div v-if="row.sku.assets">
				<span v-if="row.sku.assets.score">
				  积分：{{row.sku.assets.score}};
				</span>
				<span v-if="row.sku.assets.redpack">
				  红包：{{(row.sku.assets.redpack)}};
				</span>
				<span v-if="row.sku.assets.show_box_card">
				  显示卡：{{row.sku.assets.show_box_card}};
				</span>
				<span v-if="row.sku.assets.exclude_box_card">
				  排除卡：{{row.sku.assets.exclude_box_card}};
				</span>
				<span v-if="row.sku.assets.level_score">
				  成长值：{{row.sku.assets.level_score}};
				</span>
			</div>
