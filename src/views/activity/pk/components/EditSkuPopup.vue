<template>
  <a-modal :title="`编辑奖品`" style="top: 100px;" :visible="true" width="1000px" @cancel="handleCancel">
    ...
    <a-table class="product-table" :rowKey="row => row.id" :dataSource="list" bordered :pagination="false" size="small">
      ...
      <a-table-column title="每期抽取" width="50px">
        <template slot-scope="row">
          <span>
            {{row.per_session_stock}}件
            <!-- <a-input v-model="row.stock" placeholder="请输入" style="width:80px;" /> -->
          </span>
        </template>
      </a-table-column>
    ...
    methods: {
      ...
      handleEdit(row) {
        this.$iForm.show({
          ...
          form: row,
          success: (data) => {
            this.$http.put(`/lottery/skus/${row.uuid}`, {
              type: 'update',
              attributes: data
            }).then(res => {
              this.initList()
      ...
      handleDelete(row) {
        this.$confirm({
          ...
          content: `确认删除吗？`,
          onOk: () => {
            this.$http.delete(`/lottery/skus/${row.uuid}`).then(res => {
