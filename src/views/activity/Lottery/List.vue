    ...
    <a-table class="table" :rowKey="row => row.uuid" :dataSource="list" :pagination="pagination" :size="size">
      ...
      <a-table-column title="开奖条件" width="160px">
        <template slot-scope="row">
          ...
          <span
            v-if="row.emit_type == 'time'">{{$tool.formatDate(row.emit_time, 'MM-dd hh:mm')}}开奖</span>
          <span
            v-else-if="row.emit_type == 'user_total'">用户数达{{row.emit_user_total}}时开奖</span>
          <span
            v-else-if="row.emit_type == 'ticket_total'">抽奖码达{{row.emit_ticket_total}}个时开奖</span>

        </template>
      </a-table-column>
      <a-table-column title="次数库存" width="120px">
        <template slot-scope="row">
          <div>
              剩 {{row.stock}} 次
          </div>
      ...
      <a-table-column title="人数/抽奖码" width="120px">
        <template slot-scope="row">
          <div>
            <a href="javascript:;" @click="showUserList(row)">{{row.sales}}人参与</a>
          </div>
          <div>{{row.tickets_count}}张抽奖码</div>
        </template>
      </a-table-column>
      ...
      <a-table-column title="状态" width="60px">
        <template slot-scope="row">
          <a-tag v-if="row.union_status === 'working'" color="blue">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      fetchTotalData() {
        this.$http('/status-total/activity/lottery').then((res) => {
