...
<!--        <a-input-search placeholder="input search text" enter-button @search="onSearch" />
        ...
        <router-link to="/activity/egg-lottery/setting" @click="handleDelete(row)" style="margin: 0px 10px;">抽奖配置</router-link>
    ...
    <a-table class="table" :rowKey="row => row.uuid" :dataSource="list" :pagination="pagination" :size="size">
      ...
      <a-table-column title="次数库存/销量" width="110px">
        <template slot-scope="row">
      ...
      <a-table-column title="数据统计" width="80px">
        <template slot-scope="row">
          <div>访问数: {{row.visit_total}}</div> 
          <div>参与数: {{row.join_user_total}}</div> 
          <div>拉新数: {{row.invite_user_total}}</div> 
        </template>
      </a-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      fetchTotalData() {
        this.$http('/status-total/activity/rotate-lottery').then((res) => {
      ...
      getPath(row) {
        this.$getPathPopup.show({
          page: '/pages/rotateLottery/detail',
          param: 'uuid=' + row.uuid
        })
      },
      handleDelete(row) {
