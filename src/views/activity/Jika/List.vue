  ...
  <div class="list">
    <div class="content-head" style="margin-bottom: 20px;">
      <div class="sort-keys">
        ...
        <router-link to="/activity/jika/setting" style="margin: 0px 10px;">集卡活动配置</router-link>
    ...
    <a-table class="table" :rowKey="row => row.uuid" :dataSource="list" :pagination="pagination" :size="size">
      ...
      <a-table-column title="奖品池" width="120px">
        <template slot-scope="row">
          ...
          <div> 已兑 {{row.sku_sales}}</div>
        </template>
      </a-table-column>
      <a-table-column title="卡片池" width="120px">
        <template slot-scope="row">
          <div>
            <a href="javascript:;" @click="handleEditCard(row)">
              {{row.cards_count}} 种卡片
            </a>
          </div>
          <div>{{row.card_stock}} 库存</div>
          <div>已抽 {{row.card_sales}}</div>
        </template>
      </a-table-column>
      <!-- <a-table-column title="已抽卡数" width="120px">
        <template slot-scope="row">
          <div>{{row.sales}}张</div>
        </template>
      </a-table-column> -->
    ...
    <EditCard :info="current" v-if="isEditCard" @refresh="fetchData" @close="isEditCard = false"></EditCard>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      fetchTotalData() {
        this.$http('/status-total/activity/jika').then((res) => {
          this.statusTotal = res.data.total
        })
      },
      getPath(row) {
        this.$getPathPopup.show({
          page: '/pages/jika/detail',
          param: 'uuid=' + row.uuid
        })
      },
      handleCopy(row) {
