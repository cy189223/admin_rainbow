  ...
  <div class="list">
    <div class="content-head" style="margin-bottom: 20px;">
      <div class="sort-keys">
        ...
        <span>
          总存余:  <span class="value-font"> {{assetInfo.balance / 100}} </span>
        </span>
        <span style="margin-left: 20px;">
          总收入:  <span class="value-font"> {{assetInfo.in / 100}} </span>
        </span>
        <span style="margin-left: 20px;">
          总支出: <span class="value-font"> {{assetInfo.out / 100}} </span>
        </span>
    ...
    <a-table class="table" :rowKey="row => row.uuid" :dataSource="list" :pagination="pagination" :size="size">
      ...
      <a-table-column title="变动后余额" width="100px">
        <template slot-scope="row">
          <div>
            {{row.after / 100}} 元
          </div>
...
<script>
  ...
  const resourceType = 'asset-records/redpack'
