<template>
  <div class="list">
    <div class="content-head" style="margin-bottom: 20px;">
      <div class="sort-keys">
        <a-input placeholder="用户名/手机号/银行卡号" style="width: 180px;margin-right:10px;" v-model="key" allowClear :size="size"
          @input="onSearch" />
    ...
    <a-table class="table" :rowKey="row => row.uuid" :dataSource="list" :pagination="pagination" :size="size">
      ...
      <a-table-column title="银行名" width="100px">
        <template slot-scope="row">
          <div>{{row.bank_name}}</div>
        </template>
      </a-table-column>
      <a-table-column title="开户名" width="100px">
        <template slot-scope="row">
          <div>{{row.bank_account_name}}</div>
        </template>
      </a-table-column>
      <a-table-column title="卡号" width="100px">
        <template slot-scope="row">
          <div>{{row.bank_account_number}}</div>
        </template>
      </a-table-column>

      <a-table-column title="添加日期" dataIndex="created_at" width="80px">
...
<script>
  ...
  const resourceType = 'bank-cards'
  const resourceTypeText = '提现银行卡'
  ...
  export default {
    ...
    methods: {
      ...
      fetchData() {
        ...
        this.$http.get(`/assets/redpack`).then(res => {
