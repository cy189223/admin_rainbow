<template>
  <div class="list">
    ...
    <a-table class="table" :rowKey="row => row.uuid" :dataSource="list" :pagination="pagination" :size="size">
      <a-table-column title="套餐标题" width="80px">
        <template slot-scope="row">
          <img class="thumb" :src="row.thumb" />
          {{row.title}}
        </template>
      </a-table-column>
      <a-table-column title="价格" width="100px">
        <template slot-scope="row">
          <PriceDisplay :info="row"></PriceDisplay>
        </template>
      </a-table-column>
      <a-table-column title="描述" width="120px">
        <template slot-scope="row">
          {{row.description}}
        </template>
      </a-table-column>
      <a-table-column title="库存" width="40px">
        <template slot-scope="row">
          {{row.stock}}
        </template>
      </a-table-column>
      ...
      <a-table-column title="使用次数" width="60px">
        <template slot-scope="row">
          {{row.used_count}}次
        </template>
      </a-table-column>
...
<script>
  ...
  import EditForm from "./EditCard.vue"
  const resourceType = 'cards'
  const resourceTypeText = '卡券道具'
