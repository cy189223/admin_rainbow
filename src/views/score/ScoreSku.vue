  ...
  <div class="list">
    <div class="content-head" style="margin-bottom: 20px;">
      <div class="sort-keys">
        ...
        <router-link to="/score-sku/setting" style="margin: 0px 10px;">充值设置</router-link>
    ...
    <a-table class="table" :rowKey="row => row.uuid" :dataSource="list" :pagination="pagination" :size="size">
      <a-table-column title="封面" width="40px">
        <template slot-scope="row">
          <img class="thumb" :src="row.thumb" />
        </template>
      </a-table-column>
      <a-table-column title="积分数量" width="100px">
        <template slot-scope="row">
          {{row.score}}积分
        </template>
      </a-table-column>
      <a-table-column title="售价" width="100px">
        <template slot-scope="row">
          {{row.money_price / 100}}元
        </template>
      </a-table-column>
		<a-table-column title="赠送积分数量" width="100px">
		  <template slot-scope="row">
			{{row.extra_score}}元
		  </template>
		</a-table-column>
		<a-table-column title="赠送时间" width="100px">
		  <template slot-scope="row">
			{{row.extra_hour}}小时后
		  </template>
		</a-table-column>
      ...
      <a-table-column title="销量" width="100px">
        <template slot-scope="row">
          {{row.sales}}
        </template>
      </a-table-column>
...
<script>
  ...
  import EditForm from "./EditScoreSku.vue"
  const resourceType = 'asset/score-skus'
  const resourceTypeText = '套餐'
