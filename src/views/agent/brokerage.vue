<template>
  <div class="container">
    ...
    <a-table class="table" :rowKey="row => row.node_type" :dataSource="list">
      ...
      <a-table-column title="直接上级佣金率" width="100px">
        <template slot-scope="row">
          <div>
      ...
      <a-table-column title="间接上级佣金率" width="100px">
        <template slot-scope="row">
          <div>
...
<script>
  ...
  export default {
    ...
    created() {
      this.$http.get(`/agent/setting/brokerage`).then(res => {
    ...
    methods: {
      ...
      submit() {
        this.$http.put(`/agent/setting/brokerage`, {
          list: this.list
        }).then(res => {
